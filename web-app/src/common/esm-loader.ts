...
export const escapeHtml = (s: string) => (s + '').replace(/[&<>"']/g, function (m: string): string {
    return ({
        '&': '&amp;', '<': '&lt;', '>': '&gt;',
        '"': '&quot;', "'": '&#39;'
    })[m] || '';
...
export const importStrToExport = (url: string) => fetch(url).then(str => ESMLoader(`export const str = \`${str}\``))
// Most Advanced usecase is shim const mixinLifecycleMethods = require("./mixin-lifecycle-methods");
